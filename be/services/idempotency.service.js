...
const begin = async ({
  ...
  if (typeof key !== "string" || key.length > 200) {
    throw new ApiError(
      "Idempotency-Key must be a string of at most 200 characters.",
      400
    );
  }
...
const complete = async ({ record, statusCode, responseBody, session }) => {
  if (!record) return;
  ...
  if (result?.matchedCount === 0)
    throw new ApiError("Idempotency reservation expired. Retry checkout.", 409);
};
