    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div
        className={`admin-sidebar ${
          isSidebarOpen ? "w-64" : "w-20 is-closed"
        } bg-white duration-300 border-r border-gray-200 shadow-sm relative flex flex-col`}
      >
        {/* Sidebar Header */}
        <div className="flex items-center justify-between h-16 px-4 border-b flex-shrink-0">
          {isSidebarOpen ? (
            ...
            >
              <div className="h-8 w-8 bg-indigo-600 rounded-md flex items-center justify-center text-white font-bold text-sm">
                KE
              </div>
            ...
            <div
              className="h-8 w-8 bg-indigo-600 rounded-md flex items-center justify-center text-white font-bold text-sm mx-auto cursor-pointer"
              onClick={() => navigate("/dashboard")}
            >
              KE
            </div>
                ...
                {item.submenu ? (
                  <div>
                    <button
                      title={item.title}
                      onClick={() => {
                        if (!isSidebarOpen) navigate(item.path);
                        else toggleSubmenu(item.title);
                      }}
      ...
      <div className="admin-main flex-1 flex flex-col overflow-hidden">
        {/* Top Bar */}
        <header className="admin-topbar h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 flex-shrink-0">
          ...
          <div className="flex items-center space-x-4">
            <Link to="/shop/home" className="text-link">
              Cửa hàng
            </Link>
            <button
              title="Thông báo"
        ...
        {/* Content */}
        <main className="admin-content flex-1 overflow-y-auto p-6 bg-gray-50">
          {children}
        </main>
      </div>
    </div>
