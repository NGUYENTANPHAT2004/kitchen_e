...
exports.processChat = asyncHandler(async (req, res, next) => {
  ...
  const { message, sessionId, sessionToken, language } = req.body;
  const settings = await AISettings.current();
  if (!settings.enabled)
    return next(new ApiError("Trợ lý AI hiện chưa được bật.", 503));

  // Xác thực đầu vào
  if (typeof message !== "string" || !message.trim() || message.length > 2000) {
    return next(new ApiError("Tin nhắn cần có từ 1 đến 2.000 ký tự.", 400));
  }

  // Lấy user ID nếu đã xác thực
  const userId = req.user ? req.user.id : null;
  if (sessionId && !validSession(sessionId, userId, sessionToken))
    return next(new ApiError("Phiên trò chuyện không hợp lệ. Vui lòng bắt đầu cuộc trò chuyện mới.", 403));
  ...
  try {
    ...
    const response = await aiService.processChat(
      ...
      userId,
      chatSessionId,
      ["vi", "en"].includes(language) ? language : settings.language
    );
    ...
    response.session_token = signSession(chatSessionId, userId);
    ...
    return next(new ApiError(error.message || "Lỗi xử lý AI chat", error.statusCode || 500));
  }
});
 ...
 * @access Private
 */
exports.getPersonalizedRecommendations = asyncHandler(
  async (req, res, next) => {
    ...
    try {
      ...
      await AIAssistantLog.create({
        ...
        query: `Yêu cầu gợi ý sản phẩm cá nhân hóa${
          categoryId ? " cho danh mục " + categoryId : ""
        }`,
...
exports.getSimilarProducts = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new ApiError(error.message || "Lỗi tìm sản phẩm tương tự", 500)
    );
  }
});
...
exports.registerFace = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (storageType === "s3" && req.file.location) {
      ...
      // Xử lý ảnh local
      processedImageInfo = await imageService.processImage(
        req.file,
        "uploads/faces",
        {
  ...
  try {
    ...
    await AIAssistantLog.create({
      userId: response.success
        ? new mongoose.Types.ObjectId(response.user_id)
        : null,
      sessionId: uuidv4(),
    ...
    if (
      processedImageInfo &&
      processedImageInfo.path &&
      fs.existsSync(processedImageInfo.path)
    ) {
...
exports.getUserInsights = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new ApiError(error.message || "Lỗi lấy thông tin insight", 500)
    );
  }
});
...
exports.provideFeedback = asyncHandler(async (req, res, next) => {
  ...
  // Xác thực đầu vào
  if (typeof isHelpful !== "boolean" || !mongoose.isValidObjectId(logId) || (comments !== undefined && (typeof comments !== "string" || comments.length > 1000))) {
    return next(new ApiError("Nội dung phản hồi không hợp lệ.", 400));
  }
  ...
  try {
    ...
    const owner = logEntry.userId ? String(logEntry.userId) : null;
    if (req.user?.role !== "admin" && (owner ? req.user?.id !== owner : !validSession(logEntry.sessionId, null, req.body.sessionToken || req.get("x-chat-token"))))
      return next(new ApiError("Bạn không có quyền phản hồi hội thoại này.", 403));
    ...
    return next(
      new ApiError(error.message || "Lỗi khi cung cấp feedback", 500)
    );
  }
});
...
exports.getChatHistory = asyncHandler(async (req, res, next) => {
  ...
  if (!req.user && !validSession(sessionId, null, req.get("x-chat-token")))
    return next(new ApiError("Bạn cần đăng nhập hoặc mở đúng phiên trò chuyện.", 401));
  if (userId && !mongoose.isValidObjectId(userId))
    return next(new ApiError("Mã người dùng không hợp lệ.", 400));
  ...
  try {
    const query = req.user?.role === "admin" ? {} : { userId: req.user?._id || null };
    ...
    const formattedHistory = history.map((item) => ({
      ...
      intent_confidence: item.intentConfidence,
      suggested_products: item.suggestedProducts,
      suggested_recipes: item.suggestedRecipes,
      suggested_actions: item.suggestedActions,
    }));
    ...
    return next(
      new ApiError(error.message || "Lỗi lấy lịch sử trò chuyện", 500)
    );
  }
});
...
exports.getFrequentQueries = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const frequentQueries = await AIAssistantLog.getFrequentQueries(
      parseInt(limit) || 10
    );
    ...
    return next(
      new ApiError(error.message || "Lỗi lấy truy vấn phổ biến", 500)
    );
  }
});
...
exports.getFeedbackStats = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new ApiError(error.message || "Lỗi lấy thống kê feedback", 500)
    );
  }
});
