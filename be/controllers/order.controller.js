...
const storeSettings = require("../services/store-settings.service");
...
exports.createOrder = asyncHandler(async (req, res) => {
  ...
  if (!["cod", "bank_transfer", "vnpay"].includes(paymentMethod))
    throw new ApiError("Payment method is unavailable", 400);
  const settings = await storeSettings.getSettings();
  if (
    paymentMethod === "bank_transfer" &&
    (!settings.bankName || !settings.bankAccount || !settings.bankAccountName)
  ) {
    throw new ApiError("Bank transfer is not configured", 400);
  }
  if (paymentMethod === "vnpay" && process.env.VNPAY_ENABLED !== "true")
    throw new ApiError("VNPay is not enabled", 400);
  ...
  try {
    const cart = cartId
      ...
      : await Cart.findOne({ userId: req.user._id, status: "active" }).session(
          session
        );
    ...
    await createdOrder.populate([
      {
        path: "orderItems",
        options: { sort: { createdAt: 1 }, session },
        populate: [
          {
            path: "productId",
    ...
    // Commit the order and its replay response atomically.
    ...
    if (
      ...
      throw new ApiError(
        "Stock changed during checkout. Review your cart and retry.",
        409
      );
    }
    throw error;
...
exports.getUserOrders = asyncHandler(async (req, res) => {
  ...
  // Build query
  const isAdmin =
    ["admin", "staff"].includes(req.user.role) && req.query.scope === "admin";
  ...
  if (req.query.paymentStatus === "paid") query.isPaid = true;
  if (req.query.paymentStatus === "unpaid") query.isPaid = false;
  if (req.query.paymentStatus === "refunded") query.status = "refunded";
  if (req.query.startDate || req.query.endDate) {
    ...
    if (Object.values(query.createdAt).some((date) => !Number.isFinite(+date)))
  ...
  const orders = await Order.find(query)
    ...
    .skip(skip)
    .limit(pageSize)
    .populate("userId", "firstName lastName username email phoneNumber")
    .populate({
      path: "orderItems",
...
exports.updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  if (status === "shipped" && order.paymentMethod !== "cod" && !order.isPaid)
    throw new ApiError("Confirm payment before shipping", 409);
  if (status === "shipped")
    Object.assign(updateData, { isShipped: true, shippedAt: new Date() });
  if (status === "delivered") {
    updateData.deliveredAt = new Date();
    if (order.paymentMethod === "cod")
      Object.assign(updateData, { isPaid: true, paidAt: new Date() });
  }
  ...
  if (status === "cancelled" && order.status !== "cancelled") {
    ...
    if (!updatedOrder)
      throw new ApiError("Order changed. Reload and try again.", 409);
...
exports.requestItemRefund = asyncHandler(async (req, res) => {
  ...
  if (!["delivered", "shipped"].includes(order.status)) {
    throw new ApiError(
      `Cannot request refund for an order with status: ${order.status}`,
      400
    );
  }
  ...
  if (orderItem.refundStatus !== "none") {
    throw new ApiError(
      `Item already has a refund ${orderItem.refundStatus}`,
      400
    );
  }
