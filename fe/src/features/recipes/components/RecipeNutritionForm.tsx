...
const RecipeNutritionForm: React.FC<Props> = ({
  ...
  const productsQuery = useQuery(
    ["recipe-product-options", search],
    async () => {
      const response = await api.get("/products", {
        params: { search, limit: 30 },
      });
      return response.data.data.products as Array<{
        _id: string;
        name: string;
      }>;
    }
      ...
      <div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-5">
          {(
            ...
            ] as { key: keyof RecipeNutrition; label: string; step: string }[]
          ).map(({ key, label, step }) => (
            <div key={key}>
              <label
                htmlFor={key}
          ...
          <div className="flex flex-wrap gap-2">
            <div className="min-w-0 flex-1 space-y-2">
              ...
              >
                ...
                {products
                  .filter((product) => !relatedProducts.includes(product._id))
                  .map((product) => (
              ...
              {productsQuery.isError && (
                <button type="button" onClick={() => productsQuery.refetch()}>
                  Không tải được sản phẩm. Thử lại
                </button>
              )}
            </div>
            <button
              ...
              aria-label="Thêm sản phẩm gợi ý"
              title="Thêm sản phẩm gợi ý"
              className="self-end p-2 border border-gray-300 rounded-md disabled:opacity-40"
            >
              <Plus size={20} />
            </button>
          </div>
                ...
                {relatedProducts.map((product) => (
                  ...
                  >
                    <span className="ml-2 flex-1 w-0 truncate">
                      {products.find((entry) => entry._id === product)?.name ||
                        product}
                    </span>
                    <button
                      type="button"
