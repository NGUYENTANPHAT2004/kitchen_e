...
exports.getRecipes = asyncHandler(async (req, res) => {
  ...
  const management = ["admin", "staff"].includes(req.user?.role);
  if (management) {
  ...
  // Add other filters
  if (otherFilters.isFeatured !== undefined)
    query.isFeatured = otherFilters.isFeatured === "true";

  // Prepare options for pagination
  const options = {
    page: Math.max(1, parseInt(page, 10) || 1),
    limit: Math.min(100, Math.max(1, parseInt(limit, 10) || 12)),
    options: { includeUnpublished: management },
...
exports.getRecipe = asyncHandler(async (req, res) => {
  ...
  const includeUnpublished = ["admin", "staff"].includes(req.user?.role);

  let recipe;

  // Check if id is a valid ObjectId or a slug
  if (mongoose.Types.ObjectId.isValid(id)) {
    recipe = await Recipe.findById(id).setOptions({ includeUnpublished });
  } else {
    recipe = await Recipe.findOne({ slug: id }).setOptions({
      includeUnpublished,
    });
  }
  ...
  if (
    !recipe ||
    recipe.isDeleted ||
    (!recipe.isPublished && !includeUnpublished)
  ) {
  ...
  if (includeRelatedProducts === "true") {
    const relatedProducts = await RecipeProductLink.getProductsForRecipe(
      recipe._id
    );
    ...
    const similarRecipes = await Recipe.find({
      ...
      .select(
        "title slug coverImage difficulty preparationTime cookingTime rating"
      )
...
exports.createRecipe = asyncHandler(async (req, res) => {
  ...
  if (tags) {
    if (typeof tags === "string") {
      try {
        processedTags = tags.trim().startsWith("[")
          ? JSON.parse(tags)
  ...
  const recipeData = {
    title,
    description,
    preparationTime: preparationTime ? parseInt(preparationTime, 10) : 1,
...
exports.updateRecipe = asyncHandler(async (req, res) => {
  ...
  // Find the recipe
  const recipe = await Recipe.findById(id).setOptions({
    includeUnpublished: ["admin", "staff"].includes(req.user.role),
  });
  ...
  if (preparationTime !== undefined)
    updateData.preparationTime = parseInt(preparationTime, 10);
  if (cookingTime !== undefined)
    updateData.cookingTime = parseInt(cookingTime, 10);
  ...
  if (videoDemonstration !== undefined)
    updateData.videoDemonstration = videoDemonstration;
  ...
  if (tags) {
    if (typeof tags === "string") {
      try {
        updateData.tags = tags.trim().startsWith("[")
          ? JSON.parse(tags)
          : tags.split(",").map((tag) => tag.trim());
      } catch (_error) {
        throw new ApiError("Invalid tags format", 400);
  ...
  if (removeCoverImage === "true" && recipe.coverImage) {
    try {
      await imageService.deleteImage(
        recipe.coverImagePath || recipe.coverImage
      );
    ...
    for (const stepImageInfo of stepImagesToRemove) {
      try {
        ...
        if (instruction && instruction.image) {
          await imageService.deleteImage(
            instruction.imagePath || instruction.image
          );
      ...
      updateData.instructions = instructions.map((instruction, index) => {
        if (stepImagesMap[index]) {
          ...
          if (instruction.image) {
            imageService
              .deleteImage(instruction.imagePath || instruction.image)
              .catch((err) =>
                console.error(`Failed to delete old step image: ${err.message}`)
              );
          }
...
exports.restoreRecipe = asyncHandler(async (req, res) => {
  ...
  const recipe = await Recipe.findOne({ _id: id, isDeleted: true }).setOptions({
    includeDeleted: true,
    includeUnpublished: true,
  });
  if (!recipe) {
...
exports.manageRecipeProducts = asyncHandler(async (req, res) => {
  ...
  if (action === "remove") {
    ...
    return ApiResponse.success(
      res,
      null,
      "Product removed from recipe successfully"
    );
  } else {
...
exports.toggleFeatureRecipe = asyncHandler(async (req, res) => {
  ...
  // Find the recipe
  const recipe = await Recipe.findOne({ _id: id, isDeleted: false }).setOptions(
    { includeUnpublished: true }
  );
  if (!recipe) {
    throw new ApiError("Recipe not found", 404);
