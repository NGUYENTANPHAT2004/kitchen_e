...
describe("Pricing and promotion boundaries", () => {
    ...
    const { subtotal, lines } = pricingService.priceCartItems([
      {
        ...
        quantity: 2,
        customizations: {
          engraving: { value: "gift", priceAdjustment: 10000 },
        },
      },
    ]);
  ...
  test("applies restricted voucher only to eligible product lines", async () => {
    ...
    const result = await voucherService.validateVoucher({
      ...
      cartItems: [
        {
          productId: { _id: "eligible-product", categoryId: "cat-1" },
          lineTotal: 100000,
        },
        {
          productId: { _id: "other-product", categoryId: "cat-2" },
          lineTotal: 200000,
        },
      ],
    });
...
describe("Idempotency and gateway verification", () => {
  test("stores replay response in the checkout transaction", async () => {
    const session = { transaction: "checkout" };
    IdempotencyKey.updateOne.mockResolvedValueOnce({ matchedCount: 1 });
  ...
  test("fails checkout if its replay reservation disappeared", async () => {
    IdempotencyKey.updateOne.mockResolvedValueOnce({ matchedCount: 0 });
    await expect(
  ...
  test("verifies VNPay response signature before accepting success", () => {
    ...
    const signed = {
      ...params,
      vnp_SecureHash: paymentGatewayService.buildVnPaySignature(
        params,
    ...
    expect(
      paymentGatewayService
        .normalizeWebhook({ ...signed, vnp_ResponseCode: "24" })
        .verify()
    ).toBe(false);
  });
