  ...
  try {
    ...
    while (!emailSent && emailAttempts < maxEmailAttempts) {
      try {
        ...
        emailAttempts++;
        console.error(
          `❌ Lỗi gửi email (lần ${emailAttempts}):`,
          emailError.message
        );
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, 1000 * emailAttempts)
        );
      }
    }
...
exports.login = (req, res, next) => {
  ...
  passport.authenticate("local", async (err, user, info) => {
    ...
    if (!user) {
      return next(
        new ApiError(info.message || "Email hoặc mật khẩu không đúng", 401)
      );
    }
...
exports.updatePassword = asyncHandler(async (req, res, next) => {
  ...
  if (!currentPassword || !newPassword) {
    return next(
      new ApiError("Vui lòng cung cấp mật khẩu hiện tại và mật khẩu mới", 400)
    );
  }
  ...
  try {
    ...
    return next(
      new ApiError(
        "Cập nhật mật khẩu thành công nhưng có lỗi khi tạo phiên đăng nhập mới",
        500
      )
    );
  }
...
exports.resendVerification = asyncHandler(async (req, res, next) => {
  ...
  if (lastSent && timeDiff < minInterval) {
    ...
    return next(
      new ApiError(`Vui lòng đợi ${remainingTime} giây trước khi gửi lại`, 429)
    );
  }
  ...
  try {
    ...
    return next(
      new ApiError("Lỗi xử lý sau khi gửi email: " + error.message, 500)
    );
  }
});
...
exports.googleCallback = (req, res, next) => {
  passport.authenticate("google", { session: false }, async (err, user) => {
    ...
    try {
      ...
      return res.redirect(
        `${process.env.FRONTEND_URL}/auth/oauth-callback?token=${token}`
      );
    } catch (tokenError) {
      console.error("❌ Error generating token for Google OAuth:", tokenError);
      return res.redirect(
        `${process.env.FRONTEND_URL}/auth/login?error=token_generation_failed`
      );
    }
  })(req, res, next);
};
...
exports.facebookCallback = (req, res, next) => {
  passport.authenticate("facebook", { session: false }, async (err, user) => {
    if (err || !user) {
      ...
      return res.redirect(
        `${process.env.FRONTEND_URL}/auth/login?error=facebook_auth_failed`
      );
    }
    ...
    try {
      ...
      console.error(
        "❌ Error generating token for Facebook OAuth:",
        tokenError
      );
      return res.redirect(
        `${process.env.FRONTEND_URL}/auth/login?error=token_generation_failed`
